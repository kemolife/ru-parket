...
class BlogPost extends \yii\db\ActiveRecord
{
    ...
    public function attributeLabels()
    {
        return [
            'id' => Yii::t('blog', 'ID'),
            'catalog_id' => Yii::t('blog', 'Catalog ID'),
            'title' => Yii::t('blog', 'Title'),
            'brief' => Yii::t('blog', 'Brief'),
            'content' => Yii::t('blog', 'Content'),
            'tags' => Yii::t('blog', 'Tags'),
            'surname' => Yii::t('blog', 'Surname'),
            ...
            'commentsCount' => Yii::t('blog', 'Comments Count'),
        ];
    }
    ...
    {
        return $this->hasMany(BlogComment::className(), ['post_id' => 'id'])->orderBy(['blog_comment.created_at' => SORT_DESC])->limit(1);;
    }

    public function getStatus()
    {
        if ($this->_status === null) {
            $this->_status = new StatusBlog($this->status);
        }
        return $this->_status;
    }
    ...
    public function getTagLinks()
    {
        ...
        foreach (BlogTag::string2array($this->tags) as $tag)
            $links[] = Html::a($tag, Yii::$app->getUrlManager()->createUrl(['/blog/search', 'tag' => $tag]));
    ...
    {
        return mb_strimwidth($this->content, 0, 500, "...");
    }
